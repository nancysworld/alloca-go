...
| **INV-27** | A service unit refuses to start against an authority whose applied schema is behind the migrations the binary carries, or whose schema version cannot be read at all. Applied *ahead* is accepted, because ADR-0002's rollout migrates before the new version serves | ADR-0002, [`../design-notes/horizontal-database-authority.md`](../design-notes/horizontal-database-authority.md) §5.1 | `postgres/schema_test.go` — `TestCheckSchemaRefusesToServeAnIncompatibleAuthority` (unreadable, unapplied, behind) and `TestCheckSchemaAcceptsAnAuthorityAtOrAheadOfThisBinary`. The gate runs in `cmd/alloca-go` before the listener and the expiry worker start; `/meta` reports the value it validated rather than one it merely read |
