...
| `fixtures/` | inputs a run consumes. `deployment.json` lives here but is git-ignored — it is *observed*, so a committed copy goes stale on the next rebuild; see [`fixtures/README.md`](fixtures/README.md) | regenerated by `make topo-deployment` / `make itc-deployment` before every run |
