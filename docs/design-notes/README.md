...
- [`horizontal-database-authority.md`](horizontal-database-authority.md) — **promoted; now a pointer.** The two-phase database-scaling strategy it investigated is owned by [`../design/horizontal-database-authority.md`](../design/horizontal-database-authority.md), which retained its section numbering. The stub remains so existing links resolve; cite the formal design instead.
