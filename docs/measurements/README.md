...
| [`pr4c-quota/`](pr4c-quota/) | `aws service-quotas get-service-quota` | **Account-state evidence, not a measured run** — no workload, no measured interval, no `quotability.level`. Two verbatim captures of the applied `L-1216C47A` EC2 Standard On-Demand **vCPU** quota in `eu-west-2`, showing it fell from **5.0 to 1.0** before PR4c could provision. Both are `GetServiceQuota` responses, so both are applied values rather than pending requests. This is the external fact PR4c's `STOP / DEFER` rests on; the earlier capture includes its `eu-west-2` command and quota code but not its exact capture date, and the EC2 launch refusal itself has no artifact here |
