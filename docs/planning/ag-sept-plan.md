...
- **test PR2's falsifiable scale-out prediction as a first-class result, not as a by-product of the replica matrix.** PR2 measured this machine's ceiling at ~4,300 req/s and identified PostgreSQL's write-ahead log as what sets it, with the service at 12% CPU and the connection pool no longer binding. Its pool ladder is a scale-out experiment in disguise — from the database's side, two replicas at pool 10 resemble one replica at pool 20 — and it therefore predicts **2 replicas × pool 10 ≈ 3,060 req/s, not 2 × 2,074**. Measure it, state whether the prediction held, and if it did not, say what the pool ladder got wrong. See [`../measurements/pr2-frontier/README.md`](../measurements/pr2-frontier/README.md) §5.4;
