...
| [AG-Sept PR4 — scheduler-partitioned shard-group capacity checkpoint](reports/ag-sept-pr4-scheduler-partitioned-capacity.md) | AG-Sept | **Evidence checkpoint, not A&R.** PR4a qualified the Iteration C sustained method and PR4b executed the complete scheduler-partitioned G1/G2/G4 family. `G4_local` = 3493.9/s resolved; G1/G2 did not, so `G1_local`, `G2_local`, `E2_local`, and `E4_local` are withheld and `VAL-SCALE-6` is not discharged. Four identical G1 runs span 25.1%; Goodput tracks delivered write bandwidth at broadly stable mutations/MiB, localising the material variation to the shared write path without proving the deeper mechanism. PR4c produced no independent performance cell, so `VAL-SCALE-5` remains pending and Iteration C remains open. |
...
| [`pr4c-quota/`](pr4c-quota/) | AWS Service Quotas | Account-state evidence for the external provisioning blocker that stopped the PR4c work unit. **Not a measured run and not an Iteration C closure.** Iteration C remains open pending an equivalent independent environment; see the directory README and the PR4 checkpoint report. |
