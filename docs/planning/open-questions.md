...
**Questions.** What exact PostgreSQL resource or coordination mechanism sets the current single-authority mutation frontier? What causes the large run-to-run/environmental variance seen in retained local experiments, and are those two questions related?
...
**Promote into active work when.** A future performance decision depends on a more precise mechanism, or methodology review identifies a sufficiently discriminating investigation to justify the work. Useful progress may begin with re-analysis and better observation rather than a new implementation.
