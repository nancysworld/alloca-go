...
`transaction-semantics.md` (PR2) and ADR-0002 (PR3); this is the summary the PR
split is built on.

### 3.1 Single-unit holds (decided)

A reservation holds **one** unit of a slot's capacity. Capacity is an integer count;
there is no per-reservation quantity field in AG-M1. Shared-resource **quantities and
conserved balances are explicitly AG-M6**, modelled separately so the booking core
...
| Cancel a held / confirmed reservation | `admitted_success` | releases the unit |
...
|---|---|---|---|---|
| 1 | Timeout budget + §8.1 startup validation | `config.RequestBudget`, `config.Validate()`, `/meta` exposure, `.gitignore __debug_bin*` | no | **Draft #3** |
| 2 | Domain core + services + idempotency | `internal/domain` (entities, invariants, ports, outcome types), `internal/idempotency`, `internal/service`, in-memory repository, `transaction-semantics.md`; full unit + race tests | no | planned |
| 3 | PostgreSQL adapter | `internal/postgres` (transactional repo, `SELECT … FOR UPDATE`, per-txn `lock_timeout`/`statement_timeout`, error→outcome mapping), schema + migrations, CI Postgres integration tests, ADR-0002 | yes | planned |
...
## 6. Decisions pending ratification (before PR3)

Recommendations, not yet ratified:

- **Migrations:** `pressly/goose` with embedded SQL migrations (plain SQL, minimal
  dependencies).
- **Driver / pool:** `pgx v5` + `pgxpool`.

Both are needed only at PR3; recorded here so the choice is visible during the PR1
review and can be settled before PR3 opens. The rationale will be captured in
ADR-0002.
